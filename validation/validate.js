...
async function loadAccNameLibraries(client) {
  ...
  await client.send('Runtime.evaluate', {expression: 'const _tree = axe.utils.getFlattenedTree(document.body); let axeTargetElem;'});
  // Load aom wrapper function
  await client.send('Runtime.evaluate', {expression: 'const getAOMWrapper = async (idref) => { const aomObj = await getComputedAccessibleNode(document.getElementById(idref)); return aomObj.name;}'});
...
/**
 * Gets the AccName for node with [id='idref'] according to Chrome DevTools' implementation
 * @param {CDPSession} client 
 * @param {string} idref 
 */
async function getChromeAccName(client, idref) {
  const selectedBackendNodeId = await getBackendNodeIdFromIdref(client, idref);
  const axTree = await client.send('Accessibility.getPartialAXTree', {backendNodeId: selectedBackendNodeId});
  return axTree.nodes.filter((node) => node.backendDOMNodeId === selectedBackendNodeId)[0]?.name.value ?? '';
}
...
async function getAXEAccName(client, idref) {
  await client.send('Runtime.evaluate', {expression: 'axeTargetElem = axe.utils.querySelectorAll(_tree, \'#' + idref + '\')[0];'});
  const axeAccNameComputation = await client.send('Runtime.evaluate', {expression: 'axe.commons.text.accessibleTextVirtual(axeTargetElem);'});
...
async function getAOMAccName(client, idref) {
  const aomAccNameComputation = await client.send('Runtime.evaluate', {expression: 'getAOMWrapper(\'' + idref + '\');', awaitPromise: true});
  return aomAccNameComputation.result.value;
}

/**
 * Gets the AccName for node with [id='idref'] according to Bryan Garaventa's prototype
 * @param {CDPSession} client 
 * @param {string} idref 
 */
async function getBGAccName(client, idref) {
  const bgAccNameComputation = await client.send('Runtime.evaluate', {expression: 'getAccName(document.getElementById(\'' + idref + '\')).name;'});
...
async function getOurAccName(client, idref) {
  const ourAccNameComputation = await client.send('Runtime.evaluate', {expression: 'OurLib.getAccessibleName(document.getElementById(\'' + idref + '\'));'});
...
/**
 * Gets a string of HTML containing only the DOM nodes relevant to the computation of
 * an accessible name for the node with backendNodeId of 'nodeId'.
 * @param {CDPSession} client
...
async function getMinimisedHTML(client, idref) {
  ...
  const allRelevantNodes = await removeRedundantNodes(client, allNodesInComputation);
...
async function getAllNodesInComputation(client, nodeId) {
  ...
  while(stack && stack.length > 0) {
    ...
    const axTree = await client.send('Accessibility.getPartialAXTree', {backendNodeId: currentNodeId});
    ...
    for(let axNode of axTree.nodes) {
      ...
      else if(axNode.name && axNode.name.sources[2] && axNode.name.sources[2].nativeSourceValue) {
        nextRelatedNodes = axNode.name.sources[2].nativeSourceValue.relatedNodes;
      }
...
async function removeRedundantNodes(client, relatedNodeIds) {
  ...
  for(let currentBackendNodeId of relatedNodeIds) {
    const axTree = await client.send('Accessibility.getPartialAXTree', {backendNodeId: currentBackendNodeId});
...
async function getHTMLfromNodeIds(client, nodeIds) {
  ...
  for(let nodeId of nodeIds) {
    const result = await client.send('DOM.getOuterHTML', {backendNodeId: nodeId});
...
async function getBackendNodeIdFromIdref(client, idref) {
  ...
  const selectedNodeId = await client.send('DOM.querySelector', {nodeId: doc.root.nodeId, selector: '[id="' + idref + '"]'});
  const selectedNodeDescription = await client.send('DOM.describeNode', selectedNodeId);
